    ...
    <div class="mui-card">
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          <swiper :luobotuList="lunbotu" :isfull="false"></swiper>
        </div>
      </div>
    </div>
      ...
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          <p class="price">
            ...
            销售价：
            <span class="new_price">￥{{goodsinfo.sell_price}}</span>
          </p>
          <p>
            购买数量：
...
export default {
  data() {
    return {
      ...
      selectedCount: 1 //保存用户选中的商品数量，默认，用户买1个
    };
  },
  created() {
    this.getLuobotu();
  ...
  methods: {
    getLuobotu() {
      this.$http.get("api/getthumimages/" + this.id).then(result => {
        if (result.body.status === 0) {
          // 先循环轮播图数组的每一项，为item添加img属性，因为轮播图组件中，只认识
    ...
    enter(el, done) {
      ...
      // 获取徽标在页面中的位置
      const badgePosiotion = document
        .getElementById("badge")
        .getBoundingClientRect();

      const xDist = badgePosiotion.left - ballPosition.left;
      const yDist = badgePosiotion.top - ballPosition.top;
